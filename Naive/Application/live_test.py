        ...
        if start:
            ...
            if OBTAINED_PAYLOAD['Method'] == '0':
                ...
                try:
                    ...
                    print (mac_src + "\n" + ip_src + "\n" + str(load[srt:finish]),file = input_file)
                    print (urllib.parse.unquote(str(load[start + 1:end])) + "\n" + classed,file=input_file)
                    # print mac_src + "\n"+ ip_src + "\n" + str(load[srt:finish])+ "\n" +
                    ...
                    if not STOP_EV.is_set():
                        if add_line:
                            ...
                            add_line(urllib.parse.unquote(str(load[start + 1:end])))
                ...
                reg = load[start + 1:end] + temp[temp.rfind("\r\n"):len(temp)]
                # classify the load after removing & + = etc and substitute them by space
                classed = classifier.classify(extract_features(p.sub(' ', str(reg).lower()).split()))
                print (mac_src + "\n" + ip_src + "\n" + str(load[srt:finish]),file=input_file)
                print (urllib.parse.unquote(str(reg)) + "\n" + classed,file=input_file)
                # print mac_src + "\n" + ip_src + "\n" + str(load[srt:finish]) +
                ...
                if not STOP_EV.is_set():
                    if add_line:
                        ...
                        add_line(ip_src)
                        add_line(str(load[srt:finish]))
                        add_line(urllib.parse.unquote(str(reg)))
                        add_line(str(classed))
        else:
            pass
    else:
