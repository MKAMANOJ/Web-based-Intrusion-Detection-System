...
class DataSetManager(object):
    def __init__(self,normalFilename,anomalousFilename):
        self.normal = normalFilename
        self.anomalous = anomalousFilename
        self.merged_data = None

    def load_data_sets(self):
        self.normal = loadtxt(self.normal, delimiter=',', skiprows=1)
        self.anomalous = loadtxt(self.anomalous, delimiter=',', skiprows=1)
...
def run_svm(file1,file2):
    data_manager = DataSetManager(file1,file2)
    ...
    mysvm.set_result_file_path('result.json')
    mysvm.cross_validate_split()     

if __name__ == "__main__":
    run_svm(os.sys.argv[1],os.sys.argv[2])
